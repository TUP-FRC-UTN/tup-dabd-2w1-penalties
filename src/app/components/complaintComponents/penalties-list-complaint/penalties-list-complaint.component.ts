...
export class PenaltiesListComplaintComponent {
  ...
  applyFilter(event: Event) {
    ...
    if (filterValue) {
      this.filterComplaint = this.Complaint.filter((p) => {
        ...
        return (
          descriptionStr.includes(filterValue) 
        );
      });
    } else {
  ...
  updateDataTable() {
    ...
    let table = $('#complaintsTable').DataTable({
        ...
        columns: [
            { data: 'createdDate', render: (data) => this.formatDate(data) },
            {
                data: 'complaintState',
                render: (data) => `<button class="btn ${this.getStatusClass(data)} text-center border rounded-pill" style="width: 130px;">${data}</button>`
            },
            { data: 'description' },
            { data: 'fileQuantity', render: (data) => `<i class="bi bi-paperclip"></i> ${data} Archivo adjunto` },
            {
                ...
                <div class="btn-group gap-2">
                    <button type="button" class="btn btn-success" onclick="viewComplaint(${data.id})" style="width: 100px; height: 40px; border-radius: 15px; text-align: center;">Ver más</button>
                    <div class="dropdown">
                        <button type="button" class="btn btn-success" data-bs-toggle="dropdown" aria-expanded="false" style="width: 50px; height: 40px; border-radius: 15px; text-align: center;">•••</button>
  ...
  filterDate() {
    ...
    console.log(`Filtrando entre: ${startDate} ${endDate}`);
    this.filterComplaint = this.Complaint.filter((complaint) => {
      let complaintDate;
  
      if (Array.isArray(complaint.createdDate)) {
        const [year, month, day, hour, minute, second] = complaint.createdDate;
        complaintDate = new Date(year, month - 1, day, hour, minute, second);
      } else {
  ...
  openModal(idComplaint:number, userId:number){
    const modal = this._modal.open(PenaltiesModalStateReasonComponent, { size: 'md' ,  keyboard: false });
  ...
  viewComplaint(i:number){
    const modal = this._modal.open(PenaltiesModalConsultComplaintComponent, { size: 'xl' ,  keyboard: false });
